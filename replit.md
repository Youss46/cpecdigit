...
- **Thesis Defense Module (`Soutenances de Rapports et Mémoires`)**: Full lifecycle for academic thesis/report defense. Students submit (PDF/Word, up to 50 MB) with title, abstract, field, year. Status workflow: SOUMIS→VALIDE→PLANIFIE→SOUTENU→ARCHIVE. Admin composes jury (internal teachers or external members with email), schedules defense (date/time/room/duration). Automatic convocation email via Resend to student and external jury members on scheduling. Client-side PDF convocation generation (jsPDF). Admin records result (grade, mention, observations) after defense. Archive/library tab with filters (field, year, mention, keyword). 3 new DB tables: `memoires`, `soutenances` (UNIQUE on memoire_id), `jury_membres`. 7 API routes. Nav: "Mémoires & Soutenances" in scolarite/directeur menus; "Mes Mémoires" in student menu.
