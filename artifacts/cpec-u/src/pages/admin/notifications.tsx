...
import { Bell, CheckCheck, Loader2, MessageSquare, GraduationCap, Scale, Gavel, FileText } from "lucide-react";
...
function NotifIcon({ type }: { type: string }) {
  ...
  if (type === "memoire_soumis")  return <GraduationCap className="w-5 h-5 text-blue-500" />;
  if (type === "reclamation")     return <Scale className="w-5 h-5 text-orange-500" />;
  if (type === "jury")            return <Gavel className="w-5 h-5 text-violet-500" />;
  if (type === "grade")           return <FileText className="w-5 h-5 text-emerald-500" />;
...
export default function AdminNotificationsPage() {
  ...
  const handleMarkRead = async (n: AppNotification) => {
    ...
    if (n.type === "message")        setLocation("/admin/messages");
    if (n.type === "memoire_soumis") setLocation("/admin/memoires");
    if (n.type === "reclamation")    setLocation("/admin/reclamations");
    if (n.type === "jury")           setLocation("/admin/jury-special");
  };
  ...
  return (
    <AppLayout allowedRoles={["admin"]}>
      <div className="max-w-2xl mx-auto space-y-6 px-4 py-6">
        ...
        ) : (notifications as AppNotification[]).length === 0 ? (
