...
import {
  ...
  Clock, Users, BarChart2, Eye, BookOpen, GripVertical,
...
export default function TeacherDevoirs() {
  ...
  function resetForm() {
    setShowForm(false); setTitre(""); setDescription(""); setMatiereId("");
  ...
  function updateReponse(qi: number, ri: number, field: string, value: any) {
    setQuestions(q => q.map((qu, idx) => {
      ...
      const reponses = qu.reponses.map((r, ridx) => {
        if (ridx !== ri) {
          // Pour QCM unique et Vrai/Faux, désélectionner les autres si on sélectionne une
  ...
  function handleSubmit() {
    ...
    if (!classeIds.length) return toast({ title: "Au moins une classe requise", variant: "destructive" });
    if (!dateDebut || !dateFin) return toast({ title: "Dates requises", variant: "destructive" });
    if (questions.length === 0) return toast({ title: "Au moins une question requise", variant: "destructive" });
          ...
          <Card className="border-2 border-blue-200 shadow-lg">
            <CardHeader className="bg-blue-50 rounded-t-lg">
              <CardTitle className="text-blue-800 flex items-center gap-2">
                <ClipboardCheck className="w-5 h-5" /> Créer un nouveau devoir
              </CardTitle>
            </CardHeader>
